...
void draw_vertex(const int (*n)[2]) {
  i = NODECOUNT-1;
  do {
    display.drawPixel(n[i][0],n[i][1], SSD1306_WHITE);
  } while(i--);
}
...
// ----------------------------------------------
void draw_wireframe(const int (*n)[2]) {
  i = TRICOUNT-1;
  do {
    // don't draw triangle with negative surface value
    if (!is_hidden(n, i)) {
      // draw triangle edges - 0 -> 1 -> 2 -> 0
        display.drawLine(n[EDGE(i,0)][0], n[EDGE(i,0)][1], n[EDGE(i,1)][0], n[EDGE(i,1)][1], SSD1306_WHITE);
        display.drawLine(n[EDGE(i,1)][0], n[EDGE(i,1)][1], n[EDGE(i,2)][0], n[EDGE(i,2)][1], SSD1306_WHITE);
        display.drawLine(n[EDGE(i,2)][0], n[EDGE(i,2)][1], n[EDGE(i,0)][0], n[EDGE(i,0)][1], SSD1306_WHITE);
    }
  } while(i--);
  display.display();
}
...
void draw_flat_color(const int (*n)[2], uint16_t color) {
  ...
  do {
    ...
    if ((surface=shoelace(n, i)) < 0) {
      ...
      display.fillTriangle(n[EDGE(i,0)][0], n[EDGE(i,0)][1],
...
// display.fillRect(x0,  y0, h, w, SSD1306_INVERSE);
...
void draw_print(const int16_t color) {
  ...
  switch(draw_type) {
    case 0: display.write(" vertex");
            display.write(" count: ");
            display.write(NODECOUNT);
            break;
    case 1: display.write(" wireframe");
            display.write(" triangles: ");
            display.write(TRICOUNT);
            break;
    case 2: display.write(" flat color");
            display.write(" mask clear");
            display.write(" triangles: ");
            display.write(TRICOUNT);
            break;
    case 3: display.write(" flat color");
            display.write(" wireframe clear");
            display.write(" triangles: ");
            display.write(TRICOUNT);
            break;
    case 4: display.write(" flat color");
            display.write(" no clearscreen");
            display.write(" triangles: ");
